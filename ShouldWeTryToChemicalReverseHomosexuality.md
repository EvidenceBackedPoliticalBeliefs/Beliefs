...
We consider it morally wrong to take away someone's autonomy, even when an outside observer would confidently state they are worse off as a result. In modern western society, there are few things we value higher than personal autonomy.
*(It's arguable that we value it too highly - but for the purposes of this article we will assume this is the correct moral choice for people on average)*
...
We consider many things a *disorder* when it interrupts our ability to function well in society. For example:
...
This argument sounds appealing, but again it uses the majority state to define what *counts* as mandatory ability in order to fit in. In the autistic society described above, we can imagine there are many people who would state that the inability (or preference to avoid) to be direct when communicating, was the non-autistic person lacking the *ability* to fit in.
...
- To lack the ability to function effectively in the way the majority prefers to function *can* be deemed a mental illness.[^3]
...
[^3]: For the purposes of this discussion we only need it to be recognised that a mental disorder *can* be defined in this way. Not that all mental disorders require a particular societal context.